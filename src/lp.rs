...
impl GridProblem {
	...
	fn add_penalty_variables_big_m(&mut self) {
		...
		// and if z_ij is 1 (tower there) then p_ij will be sum of all the z_ij within
		// penalty radius of ij.
		...
		for i in 0..(self.dim as usize) {
			for j in 0..(self.dim as usize) {
				...
				self
					.constraints
					.push(constraint!(w_ij.clone() - M * (1 - delta) <= p_ij));
				self
					.constraints
					.push(constraint!(p_ij <= w_ij.clone() + M * (1 - delta)));
	...
	/// Returns (a, b) such that absolute deviation is minimized on x_min <= x <
	/// x_max for y = a * x + b and target P(x) := 170e^0.17x on integer inputs.
	...
	fn add_penalty_variables_leaky_relu(&mut self) {
		...
		// let b2: i32 = m1 * w_c + b1 - m2 * w_c; // intercept after cutoff, to make
		// function continuous
		...
		for i in 0..(self.dim as usize) {
			for j in 0..(self.dim as usize) {
				...
				self
					.constraints
					.push(constraint!(m1 * w_ij.clone() + b1 - M * (1 - eps) <= s));
				self
					.constraints
					.push(constraint!(s <= m1 * w_ij.clone() + b1 + M * (1 - eps)));
				self
					.constraints
					.push(constraint!(m2 * w_ij.clone() + b2 - M * eps <= s));
				self
					.constraints
					.push(constraint!(s <= m2 * w_ij.clone() + b2 + M * eps));
